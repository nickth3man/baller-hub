    ...
    <div className="bg-white rounded-2xl shadow-lg p-4">
      ...
      <ol className="space-y-2">
        {leaders.leaders.map((leader) => (
          <li key={leader.player_slug} className="flex items-center justify-between text-sm">
            <Link
  ...
  const [schedule, leaders] = await Promise.all([
    ...
    Promise.all(
      LEADER_CATEGORIES.map((category) =>
        getSeasonLeaders(seasonYear, { category: category.key, per_game: true, limit: 5 })
      )
    ),
  ]);
    ...
    <div className="max-w-6xl mx-auto py-12 space-y-10">
      <header className="text-center">
        ...
        <p className="mt-3 text-lg text-gray-500">
          {season.champion_team_name ? `${season.champion_team_name} were champions.` : 'Season overview.'}
        </p>
      ...
      <section>
        <h2 className="text-xl font-bold text-gray-900 mb-4">League Leaders</h2>
