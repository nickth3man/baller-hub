...
from src.data import TEAM_TO_TEAM_ABBREVIATION, PlayerData, TeamTotal
from src.errors import InvalidDate, InvalidPlayerAndSeason
...
class HTTPService:
    ...
    def play_by_play(self, home_team, day, month, year):
        ...
        url = f"{HTTPService.BASE_URL}/boxscores/pbp/{year}{add_0_if_needed(str(month))}{add_0_if_needed(str(day))}0{TEAM_TO_TEAM_ABBREVIATION[home_team]}.html"
    ...
    def team_box_score(self, game_url_path):
        url = f"{HTTPService.BASE_URL}/{game_url_path}"
    ...
    def search(self, term):
        ...
        if response.url.startswith(f"{HTTPService.BASE_URL}/search/search.fcgi"):
            ...
            while page.nba_aba_baa_players_pagination_url is not None:
                response = requests.get(
                    url=f"{HTTPService.BASE_URL}/search/{page.nba_aba_baa_players_pagination_url}"
                )
