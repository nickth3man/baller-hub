...
class Test20180101(TestCase):
    def setUp(self):
        with open(
            os.path.join(
                os.path.dirname(__file__), "../files/player_box_scores/2018/1/1.html"
            ),
            encoding="utf-8",
        ) as file_input:
...
class Test20010101(TestCase):
    def setUp(self):
        with open(
            os.path.join(
                os.path.dirname(__file__), "../files/player_box_scores/2001/1/1.html"
            ),
            encoding="utf-8",
        ) as file_input:
            self._html = file_input.read()
    ...
    def test_json_output(self, m):
        ...
        try:
            ...
            assert filecmp.cmp(output_file_path, os.path.join(os.path.dirname(__file__), "./output/expected/player_box_scores/2001/1/1.json"))
        finally:
    ...
    def test_csv_output(self, m):
        ...
        try:
            ...
            assert filecmp.cmp(output_file_path, os.path.join(os.path.dirname(__file__), "./output/expected/player_box_scores/2001/1/1.csv"))
        finally:
            os.remove(output_file_path)
