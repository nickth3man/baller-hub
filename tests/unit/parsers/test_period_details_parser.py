...
class TestPeriodDetailsParser(TestCase):
    ...
    def test_parse_period_number_is_period_count_in_regulation(
        ...
        assert PeriodDetailsParser(regulation_periods_count=4).parse_period_number(period_count="some period count") == "some period count"
    ...
    def test_parse_period_number_is_difference_between_period_count_and_regulation_periods_count(
        ...
        assert PeriodDetailsParser(regulation_periods_count=4).parse_period_number(period_count=5) == 1

    @patch.object(PeriodDetailsParser, "is_overtime")
    def test_parse_period_type_is_overtime_when_in_overtime(self, mocked_is_overtime):
        mocked_is_overtime.return_value = True
        assert PeriodDetailsParser(regulation_periods_count=4).parse_period_type(period_count="some period count") == PeriodType.OVERTIME

    @patch.object(PeriodDetailsParser, "is_overtime")
    def test_parse_period_type_is_quarter_when_in_regulation(self, mocked_is_overtime):
        mocked_is_overtime.return_value = False
        assert PeriodDetailsParser(regulation_periods_count=4).parse_period_type(period_count="some period count") == PeriodType.QUARTER
